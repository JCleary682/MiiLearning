        ...
        <section id="info" class="py-3">
            <div class="container">
                <div class='row'>
                    ...
                    <div class='col-md-6'>
                        <form enctype="multipart/form-data" action="parentprofileedit.php" method="POST" name="updateparent" id='parent-edit-form' >
                                <div class="form-group">
                                    <label for="contactName">Name </label>
                                    <input type="text" name="newfullname" class="form-control" id="parentName" placeholder="Your name"  value='<?php echo $namedata ?>' required>   
                                </div>
                                <div class="form-group">
                                    <label for="emailAddress">Email </label>
                                    <input type="email" name="newemail" class="form-control" id="parentEmail" placeholder="Your email address" value='<?php echo $emaildata ?>' required>   
                                </div>
                                <div class="form-group">
                                ...
                                <button class="btn btn-primary" type="submit" id='tutor-reg' name="updateparent">Update Details</button>
                        </form>
                        <?php 
                        if(isset($_POST['updateparent'])){
                            ...
                            //Prepare values
                            $newparentname = mysqli_real_escape_string($conn, $_POST['newfullname']);
                            $newparentemail = mysqli_real_escape_string($conn, $_POST['newemail']);
                            $newparentaddress = mysqli_real_escape_string($conn, $_POST['newaddress']);
                            $newparentusername = mysqli_real_escape_string($conn, $_POST['newusername']);
                            $newparentnumber = mysqli_real_escape_string($conn, $_POST['newnumber']);
                            $newparentdesc = mysqli_real_escape_string($conn, $_POST['newdescript']);
                            $newparentprofilepicname = $_FILES['newpropic']['name'];
                            $newparentprofilepictemp = $_FILES['newpropic']['tmp_name'];
                            $newparentpassword = mysqli_real_escape_string($conn, $_POST['newpassword1']);
                            $parentusertype = mysqli_real_escape_string($conn, $_POST['usertype']);
                            $userid = mysqli_real_escape_string($conn, $_POST['userid']);
                            
                            //Prepare Query
                            $updateparentquery = "UPDATE miiLearning_Users SET type_id=?, name=?, address=?, email_address=?, phone_number=?, password=?, username=?, profile_pic=?, tutor_descript=? WHERE id=?";
                            
                            //Initialist statement
                            $stmt = mysqli_stmt_init($conn);
                            
                            //Run statement
                            if(!mysqli_stmt_prepare($stmt, $updateparentquery)){
                                echo "SQL ERROR!";
                            } else{
                                if(!empty($newparentprofilepicname)){
                                    move_uploaded_file($newparentprofilepictemp, "../img/$newparentprofilepicname");
                                } else{
                                    echo "<p>No file selected</p>";
                                    die();
                                }
                                //Bind Params
                                mysqli_stmt_bind_param($stmt, "issssssssi", $parentusertype, $newparentname, $newparentaddress, $newparentemail ,$newparentnumber, $newparentpassword, $newparentusername, $newparentprofilepicname, $newparentdesc, $userid );
